    ...
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-lg font-semibold text-white flex items-center">
          ...
          Select Sample Register (From Order)
        </h3>
        <button
          onClick={goBack}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {regAmostrasData.map((row, idx) => {
              ...
              const refCliente = row.CDU_ModuloRefCliente || row.Ref_Descricao;
              const conector = row.CDU_ModuloRefConetorDV || row.Amostra;
              const projeto = row.CDU_ProjetoCliente || row.Projeto;
              const enc = row.cdu_projeto || row.EncDivmac;
              const clientName = row.nome || row.Cliente;
