    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-800 to-slate-900 pb-12 text-slate-200">
      <header className="bg-slate-900/80 backdrop-blur-md sticky top-0 z-10 border-b border-slate-800 px-4 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="text-center">
           <div className="text-xs text-slate-500 font-medium uppercase tracking-wider">Box Storage</div>
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-6">
        ...
        <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700 flex items-center justify-between shadow-lg">
           <div className="flex items-center gap-3">
             ...
             <div>
               <div className="text-xs text-slate-500 font-bold uppercase">Physical Coordinates</div>
               <div className="font-mono font-bold text-white text-lg">{box.cv} <span className="text-slate-600">/</span> {box.ch}</div>
             </div>
           </div>
           <div className="text-right">
             <div className="text-2xl font-bold text-white">{box.connectors.length + box.accessories.length}</div>
             <div className="text-xs text-slate-500">Total Items</div>
           </div>
        </div>

        {/* Connectors Section - Collapsible */}
        <CollapsibleSection 
          title="Connectors" 
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-lg flex items-center justify-center font-bold text-lg border ${liveStock > 0 ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                    {conn.viasCode}
                  </div>
                  <div>
                     <div className="font-mono font-bold text-white text-lg">{conn.id}</div>
                     <div className="text-sm text-slate-400">{conn.colorName} • {conn.viasName}</div>
                  </div>
                </div>
                <div className="text-right">
          ...
          >
            {box.accessories.map(acc => {
                ...
                return (
                    <div 
                      key={acc.id}
                      onClick={() => onScan(acc.id)} // Go to accessory detail
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                           ...
                           <div className="text-xs font-mono text-slate-400">For {acc.connectorId}</div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-white text-xl">{liveStock}</div>
